...
export default function CreateCampaign() {
  ...
  const dataToSend = {
    ...
    startDate: formData.startDate,    // Java sẽ nhận qua startDate
    endDate: formData.endDate,        // Java sẽ nhận qua endDate
    targetAmount: formData.targetAmount,
    image: formData.image,
    createdBy: 1 // Gửi ID của user
  };
  ...
  try {
    ...
    if (response.ok) navigate('/admin/campaigns');
  } catch (error) {
    console.error(error);
  } finally {
    setLoading(false);
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-10 text-left">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden">
        <div className="p-8 border-b border-slate-50 bg-emerald-50/30">
          <h1 className="text-2xl font-bold text-slate-900 font-serif">Create New Campaign</h1>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase ml-1"><Type className="w-3.5 h-3.5 text-emerald-500" /> Campaign Title</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase ml-1"><DollarSign className="w-3.5 h-3.5 text-emerald-500" /> Target Goal ($)</label>
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase ml-1"><MapPin className="w-3.5 h-3.5 text-emerald-500" /> Location</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase ml-1"><Calendar className="w-3.5 h-3.5 text-emerald-500" /> Start Date</label>
              <input required type="date" value={formData.startDate} onChange={e => setFormData({...formData, startDate: e.target.value})} className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all font-medium cursor-pointer" />
            </div>
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase ml-1"><Calendar className="w-3.5 h-3.5 text-emerald-500" /> End Date</label>
              <input required type="date" value={formData.endDate} onChange={e => setFormData({...formData, endDate: e.target.value})} className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all font-medium cursor-pointer" />
            </div>
          </div>

          {/* Image */}
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase ml-1"><ImageIcon className="w-3.5 h-3.5 text-emerald-500" /> Thumbnail Image URL</label>
          ...
          <div className="pt-6">
            <button disabled={loading} type="submit" className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-emerald-700 shadow-xl shadow-emerald-100 transition-all active:scale-[0.98] flex items-center justify-center gap-2">
              {loading ? "Processing..." : <><Save className="w-4 h-4" /> Publish Campaign</>}
            </button>
          </div>
        </form>
      </motion.div>
